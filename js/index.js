...
scrollContainer.addEventListener("scroll", () => {
  ...
  chart.render();
    ...
    const createRadialChart = (selector, percent, color) => {
    const options = {
      chart: {
        type: 'radialBar',
        height: '100%',
      ...
      plotOptions: {
        radialBar: {
          hollow: { size: '65%' },
          track: {
            background: '#e5e7eb' // Tailwind's gray-200 for inner grey
          },
          dataLabels: {
            name: { show: false },
            value: {
  ...
  createRadialChart("#chartHatchery", 99, "#10B981");   // Green-500
  createRadialChart("#chartBroiler", 98, "#34D399");    // Green-400
  createRadialChart("#chartPPL", 100, "#065F46");       // Green-900
